...
The LDA method approximates the Bayes classifier assuming that the $p$-dimensional random variable $X$ is drawn from a multivariate Gaussian distribution ${\mathcal N}(\mu ,\, \mathbf\Sigma)$. The classifier assigns an observation $X = x$ to the class for which
$$
...
Classical MDS rests on the following equation: Let $X$ be the $n \times p$ matrix of point coordinates (assumed here to be column-centered for simplicity); then, the matrix of squared Euclidean distances with elements $d_{ij}(\mathbf X) = \sum_{s=1}^p (x_{is} - x_{js})^2$ is
$$
